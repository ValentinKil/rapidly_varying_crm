...
from src.models.RapidCRM_auxiliary_func import RapidCRMpsi_can, lexp_can_inv
...
def step_sampling(alpha, tau, beta, c, eta, epsi, u):
    ...
    y = epsi / eta + RapidCRMpsi_can(np.array(beta), alpha, tau)
...
def RapidCRM_size_biased_sampling(alpha, tau, beta, c, eta, n):
    """
    Perform size-biased sampling using the RapidCRM algorithm.
    ...
    if tau==0 and alpha==1:
        missing_mass = missing_mass_size_sample(eta,c,n)
    else:
        warnings.warn("Missing mass is not implemented for cases where tau != 0 or alpha != 1. Setting missing_mass to 0.")
        missing_mass = 0
...
def missing_mass_size_sample(eta,c,n):
    # Return the missing mass in size sampling
    missing_mass = c * eta* (np.log(n/eta * np.log(n)) - 1)/np.log(n/eta * np.log(n))**2 # from asymptotics in section 3.5.2 - more accurate as log(n) not so large
    return missing_mass