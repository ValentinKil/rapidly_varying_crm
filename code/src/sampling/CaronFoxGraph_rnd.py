...
def CRMtoGraph(CRM, missing_mass=0, store=False, Rapid=True):
    ...
    W_star = cumsum_w[-1]  # Total sum of the weights of the GGP, before truncation
    
    if W_star == 0:
        D_star1 = 0
    else :
        D_star1 = np.random.poisson(W_star ** 2)  # Total number of directed edges between nodes w_i before truncation
        
    if missing_mass <= 0 or W_star == 0:
        D_star2 = 0
    else : 
        try :
            D_star2 = np.random.poisson(2*W_star*missing_mass) # Total number of directed edges between a node w_i and a node due to missing mass
        except ValueError:
            print('W_star:', W_star)
    ...
    if missing_mass == 0:
        ...
        D_star3 = np.random.poisson(missing_mass ** 2) # Total number of directed edges between nodes due to missing mass
    ...
    N_nodes3 = 2 * D_star3 # Total number of nodes from the missing mass with a connection with another node from the missing mass
    ...
    binedges[D_star1:D_star1+D_star2, 1] = np.linspace(N, N + D_star2 - 1, D_star2, dtype=int)
    binedges[D_star1+D_star2:, 0] = np.linspace(N + D_star2, N + D_star2 + D_star3 -1, D_star3, dtype=int)
    binedges[D_star1+D_star2:, 1] = np.linspace(N + D_star2 + D_star3, N + D_star2 + 2*D_star3 - 1, D_star3, dtype=int)
    ...
    G = coo_matrix((data, (row, col)), shape=(Nbvertices, Nbvertices)) # G is the directed multigraph
    ...
    w0 = np.zeros(Nbvertices) # nodes for which we cannot evaluate the weight are set to 0 by default - maybe should be NaN instead
