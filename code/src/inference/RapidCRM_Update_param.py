...
from src.sampling import RapidCRM_totalmass_rnd
from src.models.RapidCRM_auxiliary_func import RapidCRMpsi1
...
def update_hyper(w,s, w_rem, alpha,tau,beta,c,eta, nbMH, rw_std, estimate_beta,hyper_beta,estimate_c,hyper_c, estimate_eta,hyper_eta,rw_eta,estimate_w_rem,nmass):
    """
    Update the hyperparameters of the GGP distribution using Metropolis-Hastings algorithm.
    ...
    for _ in range(nbMH):
        ...
        if estimate_eta:
            if not rw_eta:
                try:
                    etaprop = stats.gamma.rvs(K, scale=1 / RapidCRMpsi1(2 * sum_w + 2 * w_rem, alpha ,tau, betaprop, cprop ))
                except:
                    print(RapidCRMpsi1(2 * sum_w + 2 * w_rem, alpha ,tau, betaprop, cprop ))
            else:
        ...
        #Estimate of w_rem
        if estimate_w_rem:
            wprop_rem = RapidCRM_totalmass_rnd.Rapidsumrnd(alpha, tau, betaprop + 2 *cprop* sum_w + 2 *cprop* w_rem,cprop,etaprop,nmass)
        else:
            wprop_rem=w_rem
        ...
        term1 = - ((sum_w + wprop_rem)**2 - (sum_w + w_rem)**2) #- sumall_wprop**2 + sumall_w**2 
        term2  = - (betaprop/cprop - beta/c + 2 * (w_rem - wprop_rem)) * sum_w
        if not rw_eta:
            term3 = K * (np.log(RapidCRMpsi1((2 * sum_w + 2 * wprop_rem),alpha,tau,beta,c)) - np.log(RapidCRMpsi1((2 * sum_w + 2 * w_rem), alpha, tau, betaprop, cprop)))
        else:
             term3 = K * (np.log(etaprop) - np.log(eta)) - etaprop * RapidCRMpsi1((2 * sum_w + 2 * w_rem), alpha, tau, betaprop, cprop) + eta * RapidCRMpsi1((2 * sum_w + 2 * wprop_rem), alpha, tau, beta, c)
        
        if estimate_c:
            term4 = np.sum(s) * np.log(cprop/c)+hyper_c[0]*(np.log(cprop)-np.log(c))-hyper_c[1]*(cprop-c)
            ...
            term4 = 0
        if estimate_beta:
            term5 = hyper_beta[0]*(np.log(betaprop)-np.log(beta))-hyper_beta[1]*(betaprop-beta)  #Gamma prior on beta
        else : 
            term5 = 0
