...
def Hamiltonian(N,s,u,w,logw,sum_w,w_rem,alpha,tau,beta,c,eta,pw,issimple):
    ...
    logprob = - (sum_w + w_rem)**2 - beta/c * sum_w + np.sum((N - s) * logw + s*np.log(c) - gammaln(2-s) + 4 * np.log(s) - 2 * u)
    if issimple : #If simple graph, do not take into account self-connections, i.e. w^2 in (- (sum_w + w_rem)**2)
...
def update_w(s,u,w, logw, w_rem, N, L, epsilon,alpha,tau,beta,c,eta, issimple):
    """
    Update the w of the RapidCRM distribution using Hamiltonian Monte-Carlo algorithm.
    ...
    sum_w = np.sum(w)
    H1, potential1, kinetic1=Hamiltonian(N,s,u,w,logw,sum_w,w_rem,alpha,tau,beta,c,eta,pw,issimple) # added u=log(s/(1-s)) as an argument
    H2, potential2, kinetic2=Hamiltonian(N,s,u,wprop,logwprop,sum_wprop,w_rem,alpha,tau,beta,c,eta,pwprop,issimple) # added u=log(s/(1-s)) as an argument
...
def update_s(s,u,w, logw, w_rem, N, L, epsilon,alpha,tau,beta,c,eta, issimple):
    """
    Update the s of the RapidCRM distribution using Hamiltonian Monte-Carlo algorithm.
    ...
    #Acceptance ratio
    sum_w = np.sum(w)
    H1, potential1, kinetic1=Hamiltonian(N,s,u,w,logw,sum_w,w_rem,alpha,tau,beta,c,eta,ps,issimple)# added u=log(s/(1-s)) as an argument
    H2, potential2, kinetic2=Hamiltonian(N,sprop,uprop,w,logw,sum_w,w_rem,alpha,tau,beta,c,eta,psprop,issimple)# added uprop=log(sprop/(1-sprop)) as an argument
