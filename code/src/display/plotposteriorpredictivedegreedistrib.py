...
def plot_posterior_predictive_degree(objmcmc, nburn, niter, thin, G=None, p=None, ndraws=2000, bins=16, N=10**5, plot_folder=None, fontsize=16, legend=True, dpi=300, figsize=(6, 8), xlim=(0, 2*10**3), ylim=(8*10**(-8), 1)):
    ...
    if objmcmc.prior.type == 'Rapid':
        ...
        c_samples = np.array([objmcmc.samples[i]["c"][idx]
                             for i, idx in zip(index, INDEX)])
        if p is None:
            eta_samples = np.array([objmcmc.samples[i]["eta"][idx]
                                   for i, idx in zip(index, INDEX)])
        else:
            eta_samples = np.array([objmcmc.samples[i]["eta"][idx]
                                   for i, idx in zip(index, INDEX)])*(1-p)/p
    ...
    ax.fill_between(centerbins, quantile_freq[0, :], quantile_freq[1, :],
                    color='lightblue', label='95% posterior predictive')
    ax.set_xscale('log')
    ax.set_yscale('log')

    # Plot the empirical degree distribution
    if G is not None:
        ax.loglog(emp_centerbins, emp_freq, '+',
                  markersize=10, color='tab:red', label='Data')
